...
const { getBookingDocument } = VH.dom;
const { waitForIframeCount, waitForReadyIframe } = VH.iframes;
  ...
  async handleVoyageSelection(data) {
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        await waitForIframeCount(2, VH.CONFIG.delays.maxWaitTime);

        // wait until the second iframe has its submit button or table
        await waitForReadyIframe(1, "input[type='submit']", VH.CONFIG.delays.maxWaitTime);

        // get the voyage iframe document
        const voyageDoc = this.getVoyageDocument();

        // handle TBN specially
        if (voyageValue === "TBN0125") {
