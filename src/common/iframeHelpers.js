...
(function() {
  ...
  VH.iframes = {
     ...
     * @param {number} count
     * @param {number} [maxWaitTime=CONFIG.delays.maxWaitTime] ms
     * @param {number} [checkInterval=CONFIG.delays.pollInterval] ms
     * @returns {Promise<Array<{ iframe: HTMLIFrameElement, document: Document, dialogId: string|null }>>}
     */
    waitForIframeCount(count, maxWaitTime = CONFIG.delays.maxWaitTime, checkInterval = CONFIG.delays.pollInterval) {
     ...
     */
    async waitForReadyIframe(iframeIndex = 0, selectors = [], maxWaitTime = CONFIG.delays.maxWaitTime) {
     ...
     */
    async waitForSingleIframeFullCycle(maxWaitTime = CONFIG.delays.maxWaitTime) {
